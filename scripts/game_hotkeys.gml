...
if !global.paused {
    ...
    if input_check_pressed(key_pause) if is_in_game() and room!=rmAvoidance and room!=rmBoss1 and room!=rmBoss2 and room!=rmBoss4 {
...
if keyboard_check_pressed(vk_escape) || global.close_button_pressed {
    if room == rInit || room == rmPreTitleIdk || room == rTitle {
        game_end();
    }
    else {
        restart_game();
